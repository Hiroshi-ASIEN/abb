...
void AnimationModel::Update(const char* AnimationName1, int Frame1, const char* AnimationName2, int Frame2, float BlendRatio)
{
	...
	for (auto pair : m_Bone)
	{
		...
		// 線形補完		動きが中間だった場合アニメーションの中間も作りたい
		// blendratio = 0.0 → 1 = 1.0, 2 = 0.0
		// blendratio = 0.5 → 1 = 0.5, 2 = 0.5
		// blendratio = 1.0 → 1 = 0.0, 2 = 1.0
		...
		// 回転角度 球面線形補完
		aiQuaternion rot;
		aiQuaternion::Interpolate(rot, rot1, rot2, BlendRatio); // 球面線形補完
...
aiMatrix4x4 AnimationModel::GetWorldMatrix(const char* name)
{
	BONE* bone = &m_Bone["mixamorig:RightHandThumb1"];
	return bone->WorldMatrix;
}
XMMATRIX AnimationModel::ConvertXMMatrix(const aiMatrix4x4& aiMat)
{
	return XMMATRIX(
...
void AnimationModel::Load( const char *FileName )
{
	...
	//テクスチャ読み込み
	for(unsigned i = 0; i < m_AiScene->mNumTextures; i++)
	{
