...
void Dentou::Init()
{
	...
	ScratchImage image;
//	LoadFromWICFile(L"asset\\texture\\Tree04.png", WIC_FLAGS_NONE, &metadata, image);
	...
	// ストラクチャードバッファ
	{
...

		{
			...
			for (int j = 0; j < pMax; j++)
			{
				...
				else if (j % 5 == 0)	zz *= -1.0f;

				z += zz;

				x = 1.0f * (rand() % 40 - 40);
				z = 1.0f * (rand() % 60 - 30);
				groundHeight = scene->GetGameObject<MeshField>()->GetHeight(XMFLOAT3{ x,0.0f,z });
				pos[j] = XMFLOAT3(x, groundHeight, z);
			}
		}
...
//	Renderer::GetDeviceContext()->Draw(4, 0);
		// ジオメトリインスタンシング
	Renderer::GetDeviceContext()->DrawInstanced(4, 1000, 0, 0);
...
void Dentou::SetXZPosition(XMFLOAT3 pos)
{
	Scene* scene = Manager::GetScene();
	float groundHeight;	// 地面の高さ
