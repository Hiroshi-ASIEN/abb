...
void Mosaic::Init()
{
	...
	ScratchImage image;
	LoadFromWICFile(L"asset\\texture\\mosaic.png", WIC_FLAGS_NONE, &metadata, image);
	CreateShaderResourceView(Renderer::GetDevice(), image.GetImages(), image.GetImageCount(), metadata, &m_Texture);
	assert(m_Texture);

//	Renderer::CreateVertexShader(&m_VertexShader, &m_VertexLayout, "shader\\unlitTextureVS.cso");
//	Renderer::CreatePixelShader(&m_PixelShader, "shader\\unlitTexturePS.cso");
	Renderer::CreateVertexShader(&m_VertexShader, &m_VertexLayout, "shader\\oShaderVS.cso");
	Renderer::CreatePixelShader(&m_PixelShader, "shader\\oShaderPS.cso");
...
void Mosaic::Update()
{
	// モザイク関連のパラメータを渡す
	// 画面サイズ&モザイクサイズ
//	static D3DXVECTOR4 Parameter = D3DXVECTOR4(SCREEN_WIDTH, SCREEN_HEIGHT, 64.0f, 0.0f);
	static XMFLOAT4 Parameter = XMFLOAT4(SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, 0.0f);
...
/*
	...
	{
		Parameter.z += 0.1f;
		if (Parameter.z > SCREEN_WIDTH / 4)
		{
			Parameter.z = SCREEN_WIDTH / 4;
		}
...
void Mosaic::SetTexture(ID3D11ShaderResourceView* texture)
{
	m_Texture = texture;
}

void Mosaic::InitSet(const wchar_t* texture, XMFLOAT4 size, int type)
{
	...
	// 頂点バッファ生成
	D3D11_BUFFER_DESC bd{};
	bd.Usage = D3D11_USAGE_DYNAMIC; // 動的バッファとして設定
	bd.ByteWidth = sizeof(VERTEX_3D) * 4;
	bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; // 書き込みを許可
...
void Mosaic::SetColor(XMFLOAT4 color)
{
	m_Color = color;
}

void Mosaic::SetPos(XMFLOAT4 size)
{
	VERTEX_3D vertex[4];
